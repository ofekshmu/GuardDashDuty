...
import { Users, DutySlots, DutyTypes, CoinHistory, Requests, getId, addActivity, RANKS, RANK_CSS } from '../data.js';
...
function drawPage(container, user) {
  ...
  container.innerHTML = `
    <div class="page-fade">
      <div class="page-header">
        <div>
          <div class="page-title"><i class="fa-solid fa-users"></i> Personnel Roster</div>
          <div class="page-subtitle">Manage all base personnel, ranks, and coin balances</div>
        </div>
        ${isManager ? `<button class="btn btn-primary" id="btn-add-user"><i class="fa-solid fa-user-plus"></i> Add Soldier</button>` : ''}
      </div>
      ...
      <div class="filter-bar" style="gap:10px;flex-wrap:wrap">
        ...
        <select class="form-select" id="filter-role" style="max-width:140px">
          <option value="">All Roles</option>
          <option value="user">Soldiers</option>
      ...
      <div class="user-grid" id="user-grid">
        ${renderGrid(filtered(), slots, isManager)}
      </div>
    </div>`;
  ...
  function refresh() {
    grid.innerHTML = renderGrid(filtered(), slots, isManager);
  ...
  isManager && container.querySelector('#btn-add-user')?.addEventListener('click', () => openUserModal(null, container, user));
  bindCardClicks(grid, user, container);
}
...
function renderGrid(users, slots, isManager) {
...
function openUserModal(id, container, currentUser) {
  ...
  openModal(u ? 'Edit Soldier' : 'Add Soldier', `
    <div class="form-row form-row-2">
      <div class="form-group"><label class="form-label required">Full Name</label><input class="form-input" id="um-name" value="${u?.name||''}" placeholder="e.g. Sgt. John Smith" /></div>
      <div class="form-group"><label class="form-label required">Username</label><input class="form-input" id="um-user" value="${u?.username||''}" placeholder="login handle" /></div>
    </div>
    ...
    <div class="form-row form-row-2">
      ...
      <div class="form-group">
        ...
        <select class="form-select" id="um-role">
          <option value="user"    ${u?.role==='user'   ?'selected':''}>Soldier</option>
    ...
    ${types.length ? `
    <div class="form-group">
      <label class="form-label">Duty Exemptions (user cannot be auto-matched for these)</label>
...
function saveUser(id, container, currentUser) {
  ...
  const uname  = document.getElementById('um-user').value.trim();
  const pass   = document.getElementById('um-pass')?.value;
  const rank   = parseInt(document.getElementById('um-rank').value);
  ...
  if (id) {
    ...
    const newUser = { id: getId(), username: uname, password: pass, name, role, rank: RANKS[rank], rankLevel: rank, coins: 0, capabilities: exempts, lastLogin: null, status, joinDate: new Date().toISOString().slice(0,10) };
    users.push(newUser);
    Users.set(users);
